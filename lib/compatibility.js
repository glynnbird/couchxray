...
const couchDB1 = (data) => {
  ...
  if (data.indexes.global.mangoText) {
    features.push('mango text')
  }
...
const couchDB4 = (data) => {
  ...
  if (data.indexes.reducers.custom) {
    features.push('custom reducers')
  }
  if (data.partitioned || data.indexes.partitioned.mapReduce || data.indexes.partitioned.search ||
    data.indexes.partitioned.mangoJSON || data.indexes.partitioned.mangoText) {
    features.push('partitioned databases')
  }
  if (data.scan && data.scan.docIdTooBig) {
    features.push('large document ids')
  }
  if (data.scan && data.scan.errors) {
    for (const i in data.scan.errors) {
      const e = data.scan.errors[i]
      if (e.indexKeyTooBig) {
        features.push('large index keys')
      }
      if (e.indexValueTooBig) {
        features.push('large index values')
      }
      if (e.indexValueTooBig || e.indexValueTooBig) {
        break
      }
    }
  }
